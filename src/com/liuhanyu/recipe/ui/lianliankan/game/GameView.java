...
import com.liuhanyu.recipe.ui.lianliankan.reader.GamePkg;
import com.liuhanyu.recipe.ui.lianliankan.reader.Picture;
...
public class GameView extends FrameLayout {
	...
	private Button breakCardsBtn = null, noteBtn = null, restartBtn;
	...
	public void showStartGameAlert() {
		setEnabled(false);
		new AlertDialog.Builder(getContext()).setTitle("准备好了吗？")
				.setMessage("点击“开始”按钮开始游戏").setCancelable(false)
	...
	public void breakGameCards() {
		gameCardsMap = new Card[level.getH_cards_count()][level
				.getV_cards_count()];
	...
	public void addGameCards() {
		...
		Config.setCardsOffsetX(Config.GAME_CARDS_AREA_LEFT
				+ (Config.getGameCardsAreaWidth() - Config.getCardWidth()
						* level.getH_cards_count()) / 2);
		Config.setCardsOffsetY(Config.GAME_CARDS_AREA_TOP
				+ (Config.getGameCardsAreaHeight() - Config.getCardHeight()
						* level.getV_cards_count()) / 2);
		...
		for (int i = 0; i < level.getH_cards_count(); i++) {
			for (int j = 0; j < level.getV_cards_count(); j++) {
				...
				cardsContainer.addView(card, (int) (Config.getCardWidth()),
	...
	public void genGameCards() {
		...
		gameCards.clear();
		System.out.println(level.getH_cards_count());
		System.out.println(level.getV_cards_count());
		int halfCardsCount = level.getH_cards_count()
				* level.getV_cards_count() / 2;
		Picture pic = null;
		Card card;

		for (int i = 0; i < halfCardsCount; i++) {
			pic = getGamePkg().getPictures()[(int) (Math.random() * getGamePkg()
					.getPictures().length)];
		...
		public void onClick(View v) {
			...
			if (lastCheckedCard != null) {
				if (lastCheckedCard != currentCheckedCard) {
					if (testCards()) {
						gameCardsMap[currentCheckedCard.getIndexI()][currentCheckedCard
								.getIndexJ()] = null;
						gameCardsMap[lastCheckedCard.getIndexI()][lastCheckedCard
								.getIndexJ()] = null;
						...
						if (gameCards.size() > 0) {
							if (!GameUtil.isGameConnected(level, gameCards,
									gameCardsMap, null)) {
								breakGameCards();
							}
						} else {
	...
	private boolean testCards() {
		if (lastCheckedCard.getPicture().getId() == currentCheckedCard
				.getPicture().getId()) {

			int i1 = lastCheckedCard.getIndexI(), j1 = lastCheckedCard
					.getIndexJ(), i2 = currentCheckedCard.getIndexI(), j2 = currentCheckedCard
					.getIndexJ();
	...
	private void breakGameCardsArray() {
		for (int i = 0; i < 200; i++) {
			gameCards.add(gameCards.remove((int) (Math.random() * gameCards
					.size())));
		}
	}
	...
	private final OnClickListener noteBtnClickHandler = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (gameCards.size() >= 2) {
				...
				if (GameUtil.isGameConnected(level, gameCards, gameCardsMap,
						notedCards)) {
	...
	private final OnClickListener restartBtnClickListener = new OnClickListener() {
	...
	public void setRestartBtn(Button restartBtn) {
		...
		this.restartBtn.setOnClickListener(restartBtnClickListener);
	}
